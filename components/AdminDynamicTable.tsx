...
export interface AdminDynamicTableProps<T = any> {
  data: T[]
...
export default function AdminDynamicTable<T = any>({
  data = [],
  columns,
  ...
  const autoColumns = useMemo(() => {
    return columns.map(col => ({
      ...col,
      type: col.type || detectType((data[0] as any)?.[col.key]) || 'string',
  ...
  const filtered = useMemo(() => {
    ...
    // Per-column filters
    const activeKeys = Object.keys(filters).filter(k => filters[k] !== undefined && filters[k] !== '')
      ...
      rows = rows.filter((row: any) => {
        return activeKeys.every(k => {
          ...
          switch (type) {
            ...
            case 'array':
              return Array.isArray(v) && String(v.join(',')).toLowerCase().includes(String(f).toLowerCase())
            default:
  ...
  return (
    <Box>
        ...
        {searchable && (
          <HStack flex={1} maxW="600px" gap={2}>
            <Box position="relative" flex={1}>
              <SearchIcon position="absolute" top={2.5} left={3} color={useColorModeValue('gray.400','gray.500')} />
              <Input pl={8} placeholder="Buscar..." value={search} onChange={(e) => { setSearch(e.target.value); setPage(1); }} />
            </Box>
      ...
      <Box borderWidth="1px" borderColor={borderClr} borderRadius="md" overflowX="auto">
        <Table size="sm">
          <Thead bg={headerBg}>
            <Tr>
              {autoColumns.map((col) => (
                <Th key={col.key} onClick={() => onSortClick(col.key, col.sortable)} cursor={col.sortable ? 'pointer' : 'default'} userSelect="none">
                  <HStack>
                    <Text>{col.label}</Text>
                    {sortKey === col.key ? (
                      sortDir === 'asc' ? <ChevronUpIcon boxSize={4}/> : <ChevronDownIcon boxSize={4}/>
                    ) : null}
                  </HStack>
                </Th>
              ))}
                ...
                {autoColumns.map((col) => (
                  <Th key={`filter-${col.key}`}>
                    {col.filterable === false ? null : (
                      col.type === 'boolean' ? (
                        <ChakraSelect size="xs" value={filters[col.key] ?? 'all'} onChange={(e) => { const v = e.target.value; setFilters(prev => ({ ...prev, [col.key]: v })); setPage(1); }}>
                          <option value="all">Todos</option>
                          <option value="true">Sí</option>
                          <option value="false">No</option>
                        </ChakraSelect>
                      ) : col.type === 'date' ? (
                        <Input size="xs" type="date" value={filters[col.key] ?? ''} onChange={(e) => { const v = e.target.value; setFilters(prev => ({ ...prev, [col.key]: v })); setPage(1); }} />
                      ) : (
                        <Input size="xs" placeholder={`Filtrar ${col.label}`} value={filters[col.key] ?? ''} onChange={(e) => { const v = e.target.value; setFilters(prev => ({ ...prev, [col.key]: v })); setPage(1); }} />
                      )
                    )}
                  </Th>
                ))}
                    ...
                    {col.render
                      ...
                      : col.format
                        ...
                        : (() => {
                            ...
                            if (col.type === 'array') return Array.isArray(v) ? v.join(', ') : String(v)
            ...
            {pageRows.length === 0 && (
              <Tr>
                <Td colSpan={autoColumns.length}>
                  <Box p={6} textAlign="center" color={useColorModeValue('gray.500','gray.400')}>Sin resultados</Box>
                </Td>
              </Tr>
            )}
          </Tbody>
      ...
      <HStack justify="space-between" mt={3} flexWrap="wrap" gap={3}>
        <HStack>
          <Text fontSize="sm" color={useColorModeValue('gray.600','gray.300')}>Mostrar</Text>
          <ChakraSelect size="sm" value={String(pageSize)} onChange={(e) => { setPageSize(parseInt(e.target.value, 10)); setPage(1) }} w="80px">
            {pageSizeOptions.map(s => <option key={s} value={s}>{s}</option>)}
          </ChakraSelect>
          <Text fontSize="sm" color={useColorModeValue('gray.600','gray.300')}>por página</Text>
        </HStack>
        <Text fontSize="sm" color={useColorModeValue('gray.600','gray.300')}>
          Mostrando {(page - 1) * pageSize + 1} a {Math.min(page * pageSize, filtered.length)} de {filtered.length}
        </Text>
        <HStack>
          <Button size="sm" variant="outline" onClick={() => setPage(1)} isDisabled={page === 1}>«</Button>
          ...
          <Button size="sm" variant="outline" onClick={() => setPage(totalPages)} isDisabled={page === totalPages}>»</Button>
        </HStack>
      </HStack>
    </Box>
  )
