...
export default function AdminCanjesPage() {
  ...
  const columns: ColumnConfig<Row>[] = useMemo(
            ...
            <Portal>
              ...
              >
                <MenuItem
                  ...
                  onClick={async () => {
                    try {
                      await updateEstado.mutateAsync({ canjeId: row._raw.id, estado: 'pendiente' })
                      toast({ title: 'Marcado pendiente', status: 'success' })
                    } catch (e: unknown) {
                ...
                <MenuItem
                  ...
                  onClick={async () => {
                    try {
                      await updateEstado.mutateAsync({ canjeId: row._raw.id, estado: 'entregado' })
                      toast({ title: 'Marcado entregado', status: 'success' })
                    } catch (e: unknown) {
                      const err = e as AxiosError<{ error?: string }>
                      toast({
                        title: 'Error',
                ...
                <MenuItem
                  ...
                  onClick={async () => {
                    try {
                      await updateEstado.mutateAsync({ canjeId: row._raw.id, estado: 'cancelado' })
                      toast({ title: 'Marcado cancelado', status: 'success' })
                    } catch (e: unknown) {
                      const err = e as AxiosError<{ error?: string }>
                      toast({
                        title: 'Error',
                ...
                <MenuItem
                  ...
                  onClick={async () => {
                    ...
                    if (!motivo) return
                    try {
                      await devolver.mutateAsync({ canjeId: row._raw.id, motivo })
                      toast({ title: 'Canje devuelto', status: 'success' })
                    } catch (e: unknown) {
                      const err = e as AxiosError<{ error?: string }>
                      toast({
                        title: 'Error',
                        description: err.response?.data?.error || 'No se pudo devolver',
                        status: 'error'
                      })
                    }
                  }}
    ...
    [menuBg, menuBorder, menuColor, menuHoverBg, menuShadow, toast, updateEstado, devolver]
  )
